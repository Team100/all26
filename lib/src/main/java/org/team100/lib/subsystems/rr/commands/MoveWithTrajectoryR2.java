...
import org.team100.lib.subsystems.r2.PositionSubsystemR2Adapter;
...
/**
 * Plan a trajectory in SE2 (the usual swerve way), and then follow it in R2,
 * ignoring rotation, using inverse kinematics at each step. So we're using
 * the SE2 trajectory code but not using the rotation part.
 * 
 ...
 */
public class MoveWithTrajectoryR2 extends MoveAndHold {
    ...
    public MoveWithTrajectoryR2(LoggerFactory parent, RRArm arm, Pose2d goal, double speed) {
        m_log = parent.type(this);
        m_arm = arm;
        m_goal = goal;
        // Check feasibility in constructor to avoid later exception.
        if (m_arm.config(m_goal.getTranslation()) == null)
    ...
    public void initialize() {
        Pose2d start = new Pose2d(m_arm.translation(), Rotation2d.kZero);
